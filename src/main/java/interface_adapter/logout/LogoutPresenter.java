...
public class LogoutPresenter implements LogoutOutputBoundary {
    ...
    public void prepareSuccessView(LogoutOutputData response) {
        ...
        final LoggedInState stateLoggedin = new LoggedInState(loggedInViewModel.getState());
        // 2. set the username in the state to the empty string
        stateLoggedin.setUsername("");
        // 3. set the state in the LoggedInViewModel to the updated state
        loggedInViewModel.setState(stateLoggedin);
        ...
        final LoginState stateLogin = loginViewModel.getState();
        // 6. set the username and password in the state to the empty string
        stateLogin.setUsername("");
        stateLogin.setPassword("");
        // 7. set the state in the LoginViewModel to the updated state
        loginViewModel.setState(stateLogin);
